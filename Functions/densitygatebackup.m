...
for nGate = 1:size(channels2gate,1)
    ...
    idx_keep = ~any([xdata<=0 | xdata==max(xdata), ydata<=0 | ydata==max(ydata)],2);
     assignin('base','testoutput',{idx_exclude,idx_keep});  
    ...
    events2keep = channels2gmm(nGate,2);
    ...
    if nClusters~=0
        ...
        if events2keep==0
            ...
            idx = idx_cluster==d(events2keep,2);
            nClusters=1;
        end
