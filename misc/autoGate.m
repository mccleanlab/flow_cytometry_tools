function gateOut = autoGate(varargin)

% This function automatically gates flow cytometry measurements based on
% their density but I don't really remember how it works so use at your own
% risk.
...
%     ylim(yrange)
    ...
    for n = 1:nClusters
        ...
        idx_boundary = boundary(xpoly,ypoly,0.1);
    ...
    assignin('base','test',{idx,idxOut,xdata})
    plot(pgon,'EdgeColor',[255 94 105]./255,'FaceAlpha',0,'LineWidth',1.5);
    xlabel([xlabelname newline '(' num2str(round(100*numel(idx(~isnan(idx) & idx~=0))/numel(xdata),1)) '% of events retained)']);
    pause
